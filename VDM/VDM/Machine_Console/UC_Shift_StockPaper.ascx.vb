...
#End Region
    ...
    Function GetDeviceInfo() As DataTable
        Dim SQL As String = " SELECT * FROM MS_DEVICE WHERE D_ID=" & VDM_BL.DeviceType.Printer
    ...
    Public Sub CurrentData()
        '--Current 
        Dim SQL As String = " SELECT * FROM TB_KIOSK_DEVICE WHERE KO_ID=" & BL.KioskID & " AND D_ID=" & VDM_BL.DeviceType.Printer
        Dim DA As New SqlDataAdapter(SQL, BL.ConnectionString)
        Dim DT As New DataTable
        DA.Fill(DT)
        If DT.Rows.Count > 0 Then
            If Not IsDBNull(DT.Rows(0).Item("Current_Qty")) Then
                txtMaxPaper.Text = FormatNumber(Val(DT.Rows(0).Item("Current_Qty")), 0)
            End If
        End If
    ...
    Private Sub btnInput_Full_Click(sender As Object, e As EventArgs) Handles btnInput_Full.Click
        Dim DT As DataTable = GetDeviceInfo()
        If DT.Rows.Count > 0 Then
    ...
    Function Save() As Boolean
        ...
        SQL &= " WHERE SHIFT_ID=" & Session("SHIFT_ID") & " AND D_ID=" & VDM_BL.DeviceType.Printer
        Dim DT As New DataTable
        Dim DA As New SqlDataAdapter(SQL, BL.ConnectionString)
        DA.Fill(DT)
        Dim DR As DataRow
