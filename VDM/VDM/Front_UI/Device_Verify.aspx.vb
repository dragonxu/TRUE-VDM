    ...
    Private Sub btnFace_Recognition_Click(sender As Object, e As EventArgs) Handles btnFace_Recognition.Click
        ...
        If Not IsNothing(result.response_data) Then
            If result.response_data.face_recognition_result.ToLower = "pass" Then
                ...
                'Dim SQLCUS_ID As String = "SELECT * FROM TB_CUSTOMER_DOC WHERE TXN_ID=" & TXN_ID
                'Dim DACUS_ID As New SqlDataAdapter(SQLCUS_ID, BL.ConnectionString)
                'Dim DTCUS_ID As New DataTable
                'DACUS_ID.Fill(DTCUS_ID)
                'If DTCUS_ID.Rows.Count > 0 Then
                '    'Update TB_SERVICE_TRANSACTION
                '    Dim SQL_Update As String = "UPDATE TB_SERVICE_TRANSACTION SET CUS_ID=" & DTCUS_ID.Rows(0).Item("CUS_ID") & " WHERE TXN_ID=" & TXN_ID
                '    BL.ExecuteNonQuery(SQL_Update)
                'End If
                'Dim a As Boolean = fn_Register()
